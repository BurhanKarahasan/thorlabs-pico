...
int stepsPerRev = 200;       // configurable steps per revolution
float currentSpeed = 0;      // current speed in steps/sec
float targetSpeed = 0;       // desired speed in steps/sec
float rampRate = 100;        // steps/sec^2 (acceleration)
...
void parseCommand(String cmd) {
  ...
  if (separatorIndex == -1) {
    ...
    if (command == "RPM") {
      float rpm = value.toFloat();
      targetSpeed = (rpm * stepsPerRev) / 60.0; // Convert RPM to steps/sec
      Serial.print("OK:RPM_SET:");
      Serial.println(rpm);
    }
    else if (command == "SPEED_RPS") {
      float rps = value.toFloat();
      targetSpeed = rps * stepsPerRev;
    ...
    else if (command == "STEPS_PER_REV") {
      stepsPerRev = value.toInt();
      Serial.print("OK:STEPS_PER_REV:");
...
void sendStatus() {
  float currentRPM = (currentSpeed * 60.0) / stepsPerRev;
  float targetRPM = (targetSpeed * 60.0) / stepsPerRev;
